...
/* Modified by Chris F Wood <chris.f.wood@gmail.com> to enable */
/* setting of CallerID so calls arn't from anonymous */
/* Based on app_originate.c from 11.25 and options from master */
/* along with ideas from app_dial.c on how to parse callerid */
...
#ifndef AST_MODULE
#define AST_MODULE "app_originate"
#endif
			...
			<parameter name="options" required="false">
				...
				<option name="c">
					<argument name="x" required="false" />
				 	<para>The CallerID sent will be set to <replaceable>x</replaceable>.</para>
				</option>
				</optionlist>
			</parameter>
		</syntax>
...
AST_APP_OPTIONS(originate_exec_options, BEGIN_OPTIONS
	AST_APP_OPTION_ARG('c', OPT_CALLER_ID, OPT_ARG_CALLER_ID),
END_OPTIONS );
...
static int originate_exec(struct ast_channel *chan, const char *data)
{
	...
	const char *clid_exten;
	const char *clid_name;
	...
	if (ast_test_flag64(&opts, OPT_CALLER_ID)) {
		if (!ast_strlen_zero(opt_args[OPT_ARG_CALLER_ID])) {
			char *num, *name;
			ast_callerid_parse(opt_args[OPT_ARG_CALLER_ID], &name, &num);
			clid_exten = num;
			clid_name = name;
		} else {
			char hint_clid_name[AST_MAX_EXTENSION];
			const char *clid_context;
			ast_channel_lock(chan);
			clid_context = ast_strdupa(S_OR(ast_channel_macrocontext(chan), ast_channel_context(chan)));
			clid_exten = ast_strdupa(S_OR(ast_channel_macroexten(chan), ast_channel_exten(chan)));
			ast_channel_unlock(chan);
			hint_clid_name[0] = '\0';
			clid_name = ast_get_hint(NULL, 0, hint_clid_name, sizeof(hint_clid_name), chan, clid_context,
				clid_exten) ? hint_clid_name : "";
			if (ast_strlen_zero(clid_name)) {
				clid_name = NULL;
			}
		}
	} else {
	...
	if (!strcasecmp(args.type, "exten")) {
		...
		ast_pbx_outgoing_exten(chantech, cap_slin, chandata,
				timeout * 1000, args.arg1, exten, priority, &outgoing_status, 0,
				clid_exten, clid_name, NULL, NULL, NULL, 0);
	} else if (!strcasecmp(args.type, "app")) {
		ast_debug(1, "Originating call to '%s/%s' and connecting them to %s(%s)\n",
				chantech, chandata, args.arg1, S_OR(args.arg2, ""));

		ast_pbx_outgoing_app(chantech, cap_slin, chandata,
				timeout * 1000, args.arg1, args.arg2, &outgoing_status, 0,
				clid_exten, clid_name, NULL, NULL, NULL);
	} else {
